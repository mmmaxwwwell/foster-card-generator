...
# 2. Refresh the Nix npm deps hash
...
The hash covers the entire npm dep tree derived from `package-lock.json`. `buildNpmPackage` verifies every fetched tarball against it.

### Key Nix Files

| File | Purpose |
|------|---------|
| `flake.nix` | Nix configuration, packages, dev shell. `npmDepsHash` pins the entire npm dep tree |
| `scripts/update-npm-hash.sh` | Refreshes `npmDepsHash` in `flake.nix` after `package-lock.json` changes |
...
3. Refresh Nix npm deps hash if package-lock.json changed (see above)
